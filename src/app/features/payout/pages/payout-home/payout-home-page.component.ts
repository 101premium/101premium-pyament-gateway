  ...
  template: `
    <main class="dashboard-main merchant-main grid gap-x-5 gap-y-3">
      <div class="flex flex-wrap items-end justify-between gap-x-6 gap-y-2 border-b border-[rgba(138,158,191,0.16)] pb-1.5">
        <div>
          <p class="mb-1 text-[0.8rem] font-semibold uppercase tracking-[0.06em] text-[#5b6c86]">Payout</p>
      ...
      <section class="transactions-section mt-[0.15rem] grid gap-[0.65rem]">
        <app-summary-table
          ...
          [loadingMessage]="'Loading payout transactions...'"
          [emptyTitle]="'No payout transactions found'"
          [emptyHint]="'Refine your search or return later when new payouts have been processed.'"
          [detailedState]="true"
...
export class PayoutHomePageComponent {
  ...
  protected submitPayout(): void {
    if (this.payoutSubmitting()) return;
    ...
    const payload = { reference: createUuid(), ...this.payoutForm.getRawValue() };
  ...
  private loadTransactions(searchParam: string, page: number): void {
    ...
    this.paymentsService
      .getTransactions({ searchParam, page, cryptoMode: 'DEBIT', size: this.pageSize, routePrefix: '/payout' })
  ...
  private loadPaymentAssets(): void {
    ...
    this.paymentsService
      ...
      .subscribe({
        next: (assets) => {
          this.coinOptions.set(assets.filter((a) => a.coin?.trim()));
          if (!assets.length) this.assetError.set('No coins available.');
        },
        error: (error: unknown) => {
  ...
  private loadPaymentAssetNetworks(coin: string): void {
    ...
    this.paymentsService
      ...
      .subscribe({
        next: (networks) => {
          this.networkOptions.set(networks.filter((n) => n.network?.trim()));
          if (!networks.length) this.networkError.set('No networks available for this coin.');
        },
        error: (error: unknown) => {
          this.networkOptions.set([]);
  ...
  private resolveErrorMessage(error: unknown): string {
    ...
    return 'Unable to load payout transactions right now.';
  }
